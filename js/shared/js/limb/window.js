...
Limb.Class('Limb.Window',
{
  ...
  _createWindow: function(href, windowName, createParams)
  {
    ...
    var win_params = this.params.asString();
    var win = window.open(href, this.windowName, win_params);
    return win;
  },
...
Limb.Class('Limb.Coordinates.Rect').inherits('Limb.Coordinates.Point',
{
  ...
  {
    this.x = point.x - parseInt(this.width / 2);
    this.y = point.y - parseInt(this.height / 2);
  },

  getCenter: function()
  {
    return new Limb.Coordinates.Point(this.x + parseInt(this.width / 2), this.y + parseInt(this.height / 2));
  }
});
