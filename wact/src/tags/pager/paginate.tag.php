...
class WactPaginateTag extends WactCompilerTag
{
  function generateTagContent($code)
  {
    ...
    if($this->hasAttribute('total_items'))
    {
      $total_items = $code->getTempVarRef();

      $code->writePHP($total_items.' = ');
      $code->writePHP($this->attributeNodes['total_items']->generateExpression($code));
      $code->writePHP(';');

      $code->writePhp($pager->getComponentRefCode() . '->setTotalItems(' . $total_items . ');' . "\n");
    }

    if($this->hasAttribute('limit'))
    {
      $limit = $code->getTempVarRef();

      $code->writePHP($limit.' = ');
      $code->writePHP($this->attributeNodes['limit']->generateExpression($code));
      $code->writePHP(';');

      $code->writePhp($pager->getComponentRefCode() . "->setItemsPerPage(" . $limit . ");");
    }
