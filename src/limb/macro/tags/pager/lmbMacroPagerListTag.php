...
class lmbMacroPagerListTag extends lmbMacroTag
{
    ...
    {
        $this->pager = $this->findParentByClass('limb\macro\tags\pager\lmbMacroPagerTag')->getRuntimeVar();
        ...
        $parent = $this->findParentByClass('limb\macro\tags\pager\lmbMacroPagerTag');
    ...
    {
        $elipses_tag = $this->findChildByClass('limb\macro\tags\pager\lmbMacroPagerElipsesTag');

        if ($separator_tag = $this->findChildByClass('limb\macro\tags\pager\lmbMacroPagerSeparatorTag')) {
    ...
    protected function _generateNumber($code)
    {
        ...
        if ($number_child = $this->findChildByClass('limb\macro\tags\pager\lmbMacroPagerNumberTag'))
            $number_child->generate($code);

        if ($current_child = $this->findChildByClass('limb\macro\tags\pager\lmbMacroPagerCurrentTag'))
    ...
    {
        $section_child = $this->findChildByClass('limb\macro\tags\pager\lmbMacroPagerSectionTag');
        if ($section_child)
