...
use limb\macro\src\tags\lists\lmbMacroListItemTag;

/**
 * The parent compile time component for lists
 * class limb\macro\src\tags\lists\lmbMacroListTag.
 *
 * @tag list
 * @aliases list:list
...
class lmbMacroListTag extends lmbMacroTag
{
  ...
  {
    if(!$list_item_tag = $this->findChildByClass('limb\\macro\\src\\tags\\lists\\lmbMacroListItemTag'))
      $this->raise('{{list:item}} tag is not found for {{list}} tag but required');

    $glue_tags = $list_item_tag->findImmediateChildrenByClass('limb\\macro\\src\\tags\\lists\\lmbMacroListGlueTag');
  ...
  {
    $classes = array('limb\\macro\\src\\tags\\lists\\lmbMacroListEmptyTag',
                     'limb\\macro\\src\\tags\\lists\\lmbMacroListItemTag');
  ...
  {
    if($list_empty = $this->findImmediateChildByClass('limb\\macro\\src\\tags\\lists\\lmbMacroListEmptyTag'))
    {
      $code->writePHP('if(' . $this->counter_var . ' == 0) {');
