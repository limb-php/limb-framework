...
use limb\tree\exception\lmbTreeInvalidNodeException;
use limb\tree\exception\lmbTreeConsistencyException;
 ...
 * @version $Id$
 */
class lmbMPTree implements lmbTreeInterface
{
    protected $_conn;
    ...
    {
        $existing_columns = $this->_db_table->getColumnsForSelect();
        $processed = array();
        foreach ($values as $name => $value) {
            ...
            if (in_array($column, $this->_system_columns) || !in_array($column, $existing_columns))
                continue;
    ...
    function getPathToNode($node, $ensure_node = true)
    {
        if (!is_object($node) || $ensure_node) //импликация. не выполняется только в случае is_obj = 1 и ensure = 0
            $node = $this->_ensureNode($node);

        if (!$parents = $this->getParents($node, $ensure_node = false))
            return '/';
